        ...
        try {
          ...
          if (o.usage && o.usage.total_tokens != null) {
            emit(controller, { or_usage: {
              promptTokens: o.usage.prompt_tokens || 0,
              completionTokens: o.usage.completion_tokens || 0,
              cost: typeof o.usage.cost === 'number' ? o.usage.cost : null,
            } });
          }
...
/** Label/usage/limit of the configured key — shown next to where it's saved. */
async function keyInfo() {
  const r = await fetch(`${BASE}/auth/key`, { headers: headers(), signal: AbortSignal.timeout(10000) });
  if (!r.ok) throw new Error(`OpenRouter key check failed (${r.status})`);
