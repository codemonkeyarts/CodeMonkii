 ...
 */
function importSkillArchive(file) {
  ...
  const id = slugify(prefix ? prefix.slice(0, -1) : path.basename(file).replace(/\.(skill|zip)$/i, ''));
 ...
 * reported with `already: true`. Refuses folders without a SKILL.md and
 * name collisions; heavy subfolders (node_modules, .git) are skipped.
 */
function importSkill(srcPath) {
  let src = path.resolve(srcPath);
  if (fs.statSync(src).isFile()) {
    if (/\.(skill|zip)$/i.test(src)) return importSkillArchive(src);
...
module.exports = {
  ...
  importSkill, skillDetail, skillFile,
};
